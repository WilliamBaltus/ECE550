...
module processor(
    ...
    //                  pc_next_incremented))); 
	 ...
	 assign pc_next = blt_truthy ? pc_next_branch :
				 ...
				 ((isBR & ~isBlt) ? pc_next_branch : pc_next_incremented))));
	 ...
	 assign data_writeReg = (isJal & ~overflow_alu_add_sub) ? pc_next_incremented : ((isSetx & ~overflow_alu_add_sub) ? JI_Target_Padded : write_data_reg);
...
//	 wire [31:0] immediate_sx;
//	 //wire isAdd, isSub, isAddi, isLW, isSW, isBR, isJP, isBranch, isJump;
//	 wire isAdd, isSub, isAddi, isLW, isSW, isBR, isJP, isJump, isJR;
//	 assign instruction = q_imem; // instruction is our imem value
...
//	 assign write_data_reg = overflow_alu_add_sub ? rStatus : ((Rwd & ~overflow_alu_add_sub) ? q_dmem : alu_result);
//	 assign data_writeReg = (isJal & ~overflow_alu_add_sub) ? pc_next_incremented : ((isSetx & ~overflow_alu_add_sub) ? JI_Target_Padded : write_data_reg);
//	 
//	 assign address_dmem = alu_result[11:0];
//	 assign data = data_readRegB;
